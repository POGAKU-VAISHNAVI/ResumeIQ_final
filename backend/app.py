...
# CRITICAL FIX: Stop Python writing .pyc cache files into backend/__pycache__/
# Those writes trigger Live Server to reload the browser, destroying uploaded state.
import sys
...
# FIX: uploads folder moved OUTSIDE project workspace to prevent
# Live Server from detecting file writes and reloading the page.
# Old path was ResumeIQ/uploads/ which is inside the watched workspace.
# New path is one level above the project root — never watched by Live Server.
# CRITICAL FIX: Store uploads in user home directory (~/)
# This is ALWAYS outside any VS Code workspace, regardless of where project is opened.
# Old paths like ../uploads or ../../resumeiq_uploads were still inside the watched
# workspace when VS Code was opened at a parent folder — causing Live Server reloads.
UPLOAD_FOLDER = os.path.join(os.path.expanduser('~'), 'resumeiq_uploads')
...
# Map frontend role IDs to backend keys
ROLE_ID_MAP = {
    'py':     'python_developer',
    'web':    'web_developer',
    'da':     'data_analyst',
...
def match_job():
    ...
    return ok({
        ...
        'ats_status':          {'label': ats_label, 'color': 'green' if pct >= 70 else 'amber' if pct >= 50 else 'red'},
...
if __name__ == '__main__':
    ...
    # CRITICAL FIX: use_reloader=False stops werkzeug from writing .pyc files
    # every second while watching for code changes — those writes triggered Live Server.
